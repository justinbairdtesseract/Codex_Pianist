...
parser = argparse.ArgumentParser(description="Train the 1-octave pianist policy with skrl PPO.")
...
def resolve_training_device(requested_device: str) -> str:
    ...
    try:
        ...
        print(f"[WARN] Falling back to CPU because CUDA is unavailable for PyTorch on this machine: {exc}")
        return "cpu"
...
def main():
    rl_device = resolve_training_device(args_cli.device)
    ...
    cfg["experiment"]["experiment_name"] = "ppo_1_octave_headless"
    ...
    trainer = SequentialTrainer(cfg={"timesteps": args_cli.timesteps, "headless": True}, env=env, agents=agent)
    trainer.train()
